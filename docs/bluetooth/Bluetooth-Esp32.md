...
Note that the only configuraiton stored is the setOption115 to tunr BLE on and off.  All other configurations can be set at boot if necessary using Rules.
...
BLEOp<a id="bleop"></a>|Setup and execute an active BLE operation (read/write/notify)<BR>see separate description
